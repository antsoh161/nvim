...
-- fzf-lua's own floating terminal buffer - was a lazy.nvim `keys` entry
-- gated on `ft = "fzf"` (terminal-mode only, inside fzf-lua's own window).
vim.api.nvim_create_autocmd("FileType", {
  pattern = "fzf",
  callback = function(ev)
    local o = { buffer = ev.buf, nowait = true }
    vim.keymap.set("t", "<c-j>", "<c-j>", o)
    vim.keymap.set("t", "<c-k>", "<c-k>", o)
  end,
})
...
map("n", "<leader>ff", pick("files"), { desc = "Find Files (root)" })
map("n", "<leader>fF", pick("files", { dir_picker = true }), { desc = "Find Files (cwd)" })
map("n", "<leader>ft", pick("live_grep"), { desc = "Grep (root)" })
map("n", "<leader>fT", pick("live_grep", { dir_picker = true }), { desc = "Grep (cwd)" })
map("n", "<leader>fg", "<cmd>FzfLua git_files<cr>", { desc = "Find Files (git-files)" })
map("n", "<leader>fr", "<cmd>FzfLua oldfiles<cr>", { desc = "Recent" })
map("n", "<leader>fR", pick("oldfiles", { dir_picker = true }), { desc = "Find Files (cwd)" })
-- git
